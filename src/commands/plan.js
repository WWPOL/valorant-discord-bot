    ...
    exec(message, args) {
	   ...
	   return redis.get("planning").then((res) => {
		  ...
		  return redis.set("planning", args.numPlayers).then(() => {
			 return message.util.send(`A ${args.numPlayers} player game is now being planned. Type \`!join-plan\` or \`!join-plan @user\` to add another user. Type \`!stop-plan\` to stop planning this game.`);
		  });
	   }).catch((err) => {
